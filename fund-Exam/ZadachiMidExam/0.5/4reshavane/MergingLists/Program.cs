    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if (numbers1.Count != numbers2.Count)
            {
                if (numbers1.Count > numbers2.Count)
                {
                    result.AddRange(GetReminder(numbers1, numbers2));
                }
                else
                {
                    result.AddRange(GetReminder(numbers2, numbers1));
                }
            }
            Console.WriteLine(string.Join(" ",result));
        }

        private static IEnumerable<int> GetReminder(List<int> longerList, List<int> shorterList)
        {
